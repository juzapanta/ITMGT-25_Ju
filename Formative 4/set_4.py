...
def telephone_cipher(message):
    ...
    for i in message:
        if (encoder_dict[i])[0] in prev:
            string += ('_' + encoder_dict[i])

        else:
            string += (encoder_dict[i])
...
def telephone_decipher(telephone_string):
    ...
    for i in range(len(telephone_string)):
        if telephone_string[i] == '_':
            if encoded: 
                decoded_str += decipher_dict[encoded]
                encoded = ""
        else:
            if encoded and telephone_string[i] != encoded[0]:
                decoded_str += decipher_dict[encoded]
                encoded = telephone_string[i]
            else:
                encoded += telephone_string[i]
